	...
	endpoints: (builder) => ({
		getSubAdmins: builder.query({
			...
			transformResponse: (responseData) => {
				return subAdminsAdapter.setAll(initialState, responseData);
			},
			providesTags: (result, error, arg) => [
				{ type: 'SubAdmin', id: 'LIST' },
				...result.ids.map((id) => ({ type: 'SubAdmin', id })),
			],
		}),
		addNewSubAdmin: builder.mutation({
			...
			invalidatesTags: [{ type: 'SubAdmin', id: 'LIST' }],
		}),
		updateSubAdmin: builder.mutation({
			query: (initialSubAdmin) => ({
				url: `/sub-admins/${initialSubAdmin.id}`,
				method: 'PUT',
				body: { ...initialSubAdmin },
			}),
			invalidatesTags: (result, error, arg) => [
				{ type: 'SubAdmin', id: arg.id },
			],
		}),
